...
function getThemePlayerStatusVisuals(selectedTheme: GameTheme) {
    ...
    if (selectedTheme === 'da-projects' || selectedTheme === 'foods') {
        return compactPlayerStatusVisuals;
    }

    return defaultPlayerStatusVisuals;
}
...
function createChessPiecePlayerStatusVisuals(width: number, height: number): PlayerStatusVisuals {
    return {
        orange: {
            iconMarkup: createPlayerStatusChessPieceSvg('rgba(234, 105, 0, 1)', width, height),
            counterLabel: '0',
        },
        blue: {
            iconMarkup: createPlayerStatusChessPieceSvg('rgba(9, 127, 197, 1)', width, height),
            counterLabel: '0',
        },
    };
}

function createPlayerStatusChessPieceSvg(fill: string, width: number, height: number) {
    return `<svg xmlns="http://www.w3.org/2000/svg" width="${width}" height="${height}" viewBox="0 0 22 28" fill="none" aria-hidden="true"><path d="${playerStatusChessPiecePath}" fill="${fill}"/></svg>`;
}
