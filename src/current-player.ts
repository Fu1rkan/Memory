...
const CODE_VIBES_PLAYER_COLORS: Record<PlayerColor, string> = {
  blue: 'rgba(43, 177, 255, 1)',
  orange: 'rgba(245, 142, 57, 1)',
};

const THEMED_PLAYER_BACKGROUND_COLORS: Record<PlayerColor, string> = {
  blue: '#1FAAFC',
  orange: 'rgba(234, 105, 0, 1)',
};

/** Aktualisiert das Icon fuer den aktuell aktiven Spieler. */
export function updateCurrentPlayerIndicator(
  gameScreen: HTMLElement,
  selectedTheme: GameTheme,
  currentPlayer: PlayerColor,
): void {
  const currentPlayerElement = getCurrentPlayerElement(gameScreen);
  const currentPlayerSvg = getCurrentPlayerSvg(selectedTheme, currentPlayer);

  currentPlayerElement.replaceChildren(createSvgElement(currentPlayerSvg));
}

/** Gibt das Container-Element fuer den aktuellen Spieler zurueck. */
function getCurrentPlayerElement(gameScreen: HTMLElement): HTMLElement {
...
/** Gibt das passende SVG-Markup fuer das aktive Theme zurueck. */
function getCurrentPlayerSvg(selectedTheme: GameTheme, currentPlayer: PlayerColor): string {
  if (selectedTheme === 'code-vibes') {
    return createCodeVibesPlayerSvg(CODE_VIBES_PLAYER_COLORS[currentPlayer]);
  }

  return createThemedPlayerSvg(THEMED_PLAYER_BACKGROUND_COLORS[currentPlayer]);
}

/** Erstellt das Code-Vibes-Spielericon. */
function createCodeVibesPlayerSvg(fill: string): string {
...
/** Erstellt das Spielericon fuer Gaming, DA und Food. */
function createThemedPlayerSvg(backgroundColor: string): string {
...
/** Wandelt SVG-Markup in ein echtes SVG-Element um. */
function createSvgElement(markup: string): SVGSVGElement {
  const svg = getSvgFromMarkup(markup);
...
/** Liest das erste SVG-Element aus einem Markup-String. */
function getSvgFromMarkup(markup: string): SVGSVGElement {
